...
ggml_tensor * llm_build_qwen35::build_layer_attn_linear(
    ...
    const bool emit_states   = (cparams.n_spec_max > 0)
                            && (n_seq_tokens > 1)
                            && ((uint32_t) n_seq_tokens <= 1 + cparams.n_spec_max);
    ...
    // The conv state after t tokens (1-indexed) is the (kernel_size-1)-wide
    // window of conv_input starting at offset t — i.e., it's a pure layout
    // operation (no kernel change needed for ssm_conv).
    if (!emit_states) {
        // Single update: state after T tokens -> slot 0 (active/committed).
        ...
        // Multi-snapshot: state after t tokens -> slot (T-t) for t=1..T.
        // Slot T (after 1 token) goes to (T-1); slot 1 (after T-1 tokens)
        // goes to slot 1; slot 0 (after T tokens) goes to slot 0 (final).
        const size_t row_size = (conv_kernel_size - 1) * conv_channels * ggml_element_size(conv_states_all);
        for (int64_t t_step = 1; t_step <= n_seq_tokens; ++t_step) {
            const uint32_t slot = (uint32_t)(n_seq_tokens - t_step);
            ggml_tensor * conv_state_t =
                ggml_view_3d(ctx0, conv_input, conv_kernel_size - 1, conv_channels, n_seqs,
                             conv_input->nb[1], conv_input->nb[2],
                             t_step * ggml_element_size(conv_input));
            ggml_tensor * dst =
                ggml_view_2d(ctx0, conv_states_all, (conv_kernel_size - 1) * conv_channels, n_seqs,
                             conv_states_all->nb[1],
                             ((size_t) slot * mem_size + kv_head) * row_size);
            ggml_build_forward_expand(gf, ggml_cpy(ctx0, conv_state_t, dst));
        }
    }
    ...
    // note: need explicit repeat only if we are not using the fused GDN. The
    // emit op uses the same fused kernel and handles the broadcast itself.
    if (num_k_heads != num_v_heads && !emit_states && (!cparams.fused_gdn_ar || !cparams.fused_gdn_ch)) {
    ...
    if (!emit_states) {
        ...
        // Emit per-token state snapshots packed in the output tensor and
        // route each into the corresponding rollback slot in s_l.
        // The op flags op_params[0]=1 so the CPU kernel writes T snapshots
        // to the state region instead of just the final.
        ggml_tensor * gdn_out = ggml_gated_delta_net_emit(
            ctx0, q_conv, k_conv, v_conv, gate, beta, state);
        cb(gdn_out, "gdn_emit", il);
        ...
        // Attn-output view (same layout as build_delta_net_fused).
        ...
        // Route each per-token snapshot to its slot. emit[t] (0-indexed,
        // = state after token t+1) goes to slot (T-1-t). Slot 0 ends up
        // with emit[T-1] = final, matching non-emit semantics.
        const size_t row_size = hparams.n_embd_s() * ggml_element_size(ssm_states_all);
        for (int64_t t = 0; t < n_seq_tokens; ++t) {
            const uint32_t slot = (uint32_t)(n_seq_tokens - 1 - t);
            ggml_tensor * snap = ggml_view_4d(ctx0, gdn_out,
                S_v, S_v, H_v, n_seqs,
                ggml_row_size(gdn_out->type, S_v),
                ggml_row_size(gdn_out->type, S_v * S_v),
                ggml_row_size(gdn_out->type, S_v * S_v * H_v),
                ggml_row_size(gdn_out->type, attn_score_elems + t * state_size_per_snap));

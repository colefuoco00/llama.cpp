...
// Tiled flash attention implementation using ggml_type for K/V type dispatch
static void ggml_compute_forward_flash_attn_ext_tiled(
    ...
    // K and V must have the same type
    GGML_ASSERT(k->type == v->type);
    const ggml_type kv_type = k->type;

    // Get type traits for conversions and vec_dot
    ...
    while (ir < ir1) {
        ...
        // Load and convert Q tile to KV type
        ...
        for (int64_t ic = 0; ic < nek1; ic += KV_TILE_SZ) {
            ...
            for (int tq = 0; tq < Q_TILE_SZ; tq++) {
                ...
                // Rescale previous accumulator if we have a new max
                if (Mnew > Mold) {
        ...
        // Normalize and write output only for valid rows in the tile
        for (int tq = 0; tq < tile_rows; tq++) {
            // V /= S
...
static void ggml_compute_forward_flash_attn_ext_f16(
    ...
    // Check if we can use the tiled path
    // Requires: Q is F32, K/V are F32 or F16, KV length divisible by tile size
    static constexpr int64_t KV_TILE_SZ = ggml_fa_tile_config::KV;
    static constexpr int64_t Q_TILE_SZ  = ggml_fa_tile_config::Q;
    const bool kv_is_f32_or_f16 = (k->type == GGML_TYPE_F32 || k->type == GGML_TYPE_F16);
    // Only use tiled path for larger batch sizes (prefill) where tiling is beneficial
    // For small nb (decode), the non-tiled path is faster
    const bool use_tiled = (q->type == GGML_TYPE_F32 &&
                            kv_is_f32_or_f16 &&
                            k->type == v->type &&  // K and V must have same type
                            nek1 % KV_TILE_SZ == 0 &&

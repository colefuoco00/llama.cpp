...
#pragma unroll
        for (int i = 0; i < experts_per_thread; i++) {
            ...
            if (idx < n_expert_used) {
                selected_max = max(selected_max, output_weights[i]);
            }
        }

        selected_max = warp_reduce_max(selected_max);
...
#pragma unroll
        for (int i = 0; i < experts_per_thread; i++) {
            const int idx = i * WARP_SIZE + threadIdx.x;
            if (idx < n_expert_used) {
                const float val   = expf(output_weights[i] - selected_max);
                output_weights[i] = val;
                sum += val;
            }
        }
